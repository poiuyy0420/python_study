...
# 서브루틴 : 메인루틴에서 -> 리턴에 의해 호출 부분으로 돌아와 다시 프로세스 시작
# 코루틴 : 루틴 실행 중 멈춤 가능 -> 특정 위치로 돌아갔다가 -> 다시 원래 위치로 돌아와 수행 가능 ->  동시성 프로그래밍 가능
# 코루틴 : 코루틴 스케줄링 오버헤드 매우 적다. (하나의 쓰레드에서 실행 되기 때문에)
...
# 잘못된 사용
c2 = coroutine1()
# c2.send(100) # 예외 발생(next 없기 때문에)


# 코루틴 예제2
# GEN_CREATED : 처음 대기 생태
# GEN_RUNNING : 실행 상태
...
def coroutine2(x):
    ...
    y = yield x
    print('>>> coroutine receibed 1 : {}'.format(y))
    z = yield x + y
    print('>>> coroutine receibed 2 : {}'.format(z))
